...
if (confirm("Почати тестування?")) {
  ...
  // 1. Загальна вартість
  let totalPrice = dataList.reduce((total, item) => total + item.price, 0);
  console.log("Загальна вартість:", totalPrice);

  // 2. Кількість товарів зі зниженою ціною
  let discountedItemsCount = dataList.filter(
    (item) => item.price < item.old_price
  ).length;
  console.log("Кількість товарів зі зниженою ціною:", discountedItemsCount);

  // 3. Товари, які доступні
  let availableItems = dataList.filter(
    (item) => item.sell_status === "available"
  );
  console.log("Доступні товари:", availableItems);

  // 4. Новий список об'єктів доступних товарів
  let availableItemsWithDetails = availableItems.map((item) => ({
    id: item.id,
    price: item.price,
  ...
  console.log(
    "Список доступних товарів з потрібними полями:",
    availableItemsWithDetails
  );
}
